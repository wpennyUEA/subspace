function [net2,theta] = bnn_init_net2 (net1,lambda)
% Initialise second (augmented) neural net
% FORMAT [net2,theta] = bnn_init_net2 (net1,lambda)
%
% net1          Fitted net1 model
% lambda        .back    from task2 hidden to task1 out
%               .within  within task two
%               .forward from task1 hidden to task2 out
...
new.W1 = [1 1; 1 -1];
new.b1 = [-6,0]';
new.back_W2 = zeros(net1.H2,new.H1);
new.transfer_W2 = zeros(new.H2,net1.H1);
new.within_W2 = zeros(new.H2,new.H1);
new.b2 = zeros(new.H2,1);
new.w = zeros(new.H2,1);
new.b = 0;

net2 = bnn_augment_net (net1,new);
...
Lnew = new;
Lnew.W1 = lambda.within*ones(new.H1,net1.D);
Lnew.b1 = lambda.within*ones(new.H1,1);
Lnew.back_W2 = lambda.back*ones(net1.H2,new.H1);
Lnew.transfer_W2 = lambda.forward*ones(new.H2,net1.H1);
Lnew.within_W2 = lambda.within*ones(new.H2,new.H1);
Lnew.b2=lambda.out*ones(new.H2,1);
Lnew.w = lambda.out*ones(new.H2,1);
Lnew.b = lambda.out;

Lambda1 = bnn_unpack(net1.lambda_post,net1,'weights');
Lambda2 = bnn_augment_net (Lambda1,Lnew);
